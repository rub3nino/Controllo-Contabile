...
import { ControlloDashboard } from "../domain";
import { JetDashboard } from "../jet";
...
function AppContent() {
  ...
  return (
    <div className="flex h-dvh min-h-0 overflow-hidden bg-surface text-ink-body antialiased">
      {/* ─── Sidebar ─── */}
      ...
      >
        <a
          href="#quadra-main"
          className="sr-only focus:not-sr-only focus:absolute focus:z-50 focus:top-2 focus:left-2 focus:px-3 focus:py-2 focus:bg-surface-card focus:text-sm focus:text-ink-primary focus:border focus:border-border-subtle focus:rounded-md"
        >
          Vai al contenuto
        </a>
        {/* ─── Top bar with breadcrumb ─── */}
        <header className="sticky top-0 z-40 h-10 px-5 bg-surface/90 backdrop-blur-sm border-b border-border-muted flex items-center justify-between flex-shrink-0">
          <nav aria-label="Percorso" className="flex items-center gap-1.5 text-xs text-ink-secondary min-w-0">
            {breadcrumb.map((item, index) => (
              <span key={index} className="flex items-center gap-1.5 min-w-0">
                {index > 0 && <span className="text-[#c8c7c3]">/</span>}
                <span
                  className={`truncate ${index === breadcrumb.length - 1 ? "text-ink-body font-medium" : "hover:text-ink-body"}`}
                >
                  {item}
                </span>
              </span>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <div className="hidden md:flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-medium bg-surface-hover text-[#55534e] border border-[#e5e4df]">
              <Icon name="verified" size="sm" className="text-emerald-700" />
              <span>Conforme ISA / SA Italia</span>
            </div>
            <div className="h-3.5 w-px bg-[#e3e2de]" />
            <button
              type="button"
              className="px-2 py-1 text-xs text-ink-secondary hover:text-ink-body hover:bg-surface-sidebar-hover rounded transition-colors flex items-center gap-1"
              title="Condividi"
            >
            ...
            <button
              ...
              className="px-2 py-1 text-xs text-ink-secondary hover:text-ink-body hover:bg-surface-sidebar-hover rounded transition-colors"
              title="Altro"
            >
        ...
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col relative">
          <div className="absolute inset-0 notion-grid-bg pointer-events-none" aria-hidden="true" />
          <div
            id="quadra-main"
            className={`relative z-[1] flex-1 min-h-0 w-full min-w-0 flex flex-col ${
              activeSection === "jet" ? "" : "px-5 lg:px-6 py-3"
            }`}
          >
            ...
            {activeSection === "jet" && (
              <div className="flex-1 min-h-0 overflow-y-auto px-8 sm:px-12 lg:px-16 py-8">
                <JetDashboard />
              </div>
            )}
...
function ControlloContabileSection(props: ControlloContabileSectionProps) {
  ...
  return (
    <div className="space-y-4 min-w-0 w-full h-full overflow-y-auto">
      <button
        type="button"
        onClick={() => setView("overview")}
        className="inline-flex items-center gap-1.5 h-9 px-2 -ml-2 rounded-md text-sm text-ink-secondary hover:text-ink-primary hover:bg-surface-hover"
      >
